...
const getComposersByLastNameLetter = async(req, res) => {
    try {
        ...
        const composers = await Composer.find({})
        const filtered = composers.filter(item => item.last_name.toUpperCase().startsWith(letter.toUpperCase()))
...
const getComposerByTime = async(req, res) => {
    try {
        ...
        const timePeriod = await Time.find({period: {$regex: caseInsensitive(time)}})
        console.log(timePeriod)
        const composers = await Composer.find({time_period: timePeriod[0]._id})
